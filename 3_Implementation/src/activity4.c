...
{ 
    int i=0;
        if(temp>=0 && temp<=200){
            ...
            i=0;
            while(data2[i]!=0){
            while (!( UCSR0A & (1<<UDRE0)));  // here Wait for empty transmit buffer
            UDR0 = data2[i];        // here Put data into buffer, sends the data
            i++;
            }
            _delay_ms(2000);
        }
            ...
            while(data3[i]!=0){
            while (!( UCSR0A & (1<<UDRE0)));  // here Wait for empty transmit buffer
            UDR0 = data3[i];        // here Put data into buffer, sends the data
            i++;
            }
            _delay_ms(2000);
        }
            ...
            while(data4[i]!=0){
            while (!( UCSR0A & (1<<UDRE0)));  // here Wait for empty transmit buffer
            UDR0 = data4[i];        // here Put data into buffer, sends the data
            i++;
            }
            _delay_ms(2000);
        }
            ...
            while(data5[i]!=0){
            while (!( UCSR0A & (1<<UDRE0)));  // here Wait for empty transmit buffer
            UDR0 = data5[i];        //here  Put data into buffer, sends the data
            i++;
            }
            _delay_ms(2000);
